...
class TMRpcm {
   public:
    ...
    void precisePlay(char* filename, float startingSecond);
    void precisePlay(char* filename, unsigned long seekPoint);
...
//*** MULTI MODE **
   ...
   private:
    ...
    byte lastSpeakPin;
    byte metaInfo(boolean infoType, char* filename, char* tagData,
                  byte whichInfo);
