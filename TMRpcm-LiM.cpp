...
const byte togByte =
    _BV(ICIE1);  // Get the value for toggling the buffer interrupt on/off
...
//*** These aliases can be commented out to enable full use of alternate timers
// for other things
...
#elif defined(__AVR_ATmega32U4__) || (__AVR_AT90USB646__) || \
    defined(__AVR_AT90USB1286__) || (__AVR_ATmega128__) ||   \
...
volatile boolean buffEmpty2[2] = {false, false}, whichBuff2 = false,
                 playing2 = 0;
...
volatile boolean buffEmpty[2] = {true, true}, whichBuff = false, playing = 0, a,
                 b;

//*** Options/Indicators from MSb to LSb: paused, qual, rampUp, 2-byte samples,
// loop, loop2nd track, 16-bit ***
...
void TMRpcm::timerSt() {
    *ICRn[tt] = resolution;
#if !defined(DISABLE_SPEAKER2)
    *TCCRnA[tt] = _BV(WGM11) | _BV(COM1A1) | _BV(COM1B0) |
                  _BV(COM1B1);  // WGM11,12,13 all set to 1 = fast PWM/w ICR TOP
#else
    *TCCRnA[tt] = _BV(WGM11) |
                  _BV(COM1A1);  // WGM11,12,13 all set to 1 = fast PWM/w ICR TOP
#endif
    *TCCRnB[tt] = _BV(WGM13) | _BV(WGM12) | _BV(CS10);
#if defined(MODE2)
    *ICRn[tt2] = resolution;
    *TCCRnA[tt2] =
        _BV(WGM11) | _BV(COM1A1) | _BV(COM1B0) |
        _BV(COM1B1);  // WGM11,12,13 all set to 1 = fast PWM/w ICR TOP
...
void TMRpcm::timerSt() {
    *TCCRnA[tt] =
        _BV(WGM21) | _BV(WGM20) | _BV(COM2B1);  // Fast PWM with 0xFF (255) top
}
#endif
...
void TMRpcm::loop(boolean set) { bitWrite(optionByte, 3, set); }
/**************************************************
...
boolean TMRpcm::seek(unsigned long pos) { return sFile.seek(pos); }

unsigned long TMRpcm::fPosition() { return sFile.position(); }
...
boolean TMRpcm::seek(unsigned long pos) { return sFile.seekSet(pos); }

unsigned long TMRpcm::fPosition() { return sFile.curPosition(); }

boolean TMRpcm::ifOpen() { return sFile.isOpen(); }
...
#if !defined( \
    ENABLE_MULTI)  // Below section for normal playback of 1 track at a time

void TMRpcm::play(char* filename) { play(filename, 0); }

void TMRpcm::precisePlay(char* filename, unsigned long seekPosition) {
    ...
    if (seekPosition > 0) {
        ...
        /** Original function
        if(seekPosition > 0){ seekPosition = (SAMPLE_RATE*seekPosition) +
        fPosition();
                seek(seekPosition); //skip the header info
         */
    }
...
void TMRpcm::setVolume(char vol) { volMod = vol - 4; }
...
ISR(TIMER2_COMPB_vect) {
    ...
    // The first step is to disable this interrupt before manually enabling
    // global interrupts. This allows this interrupt vector (COMPB) to continue
    // loading data while allowing the overflow interrupt to interrupt it. (
    // Nested Interrupts ) TIMSK1 &= ~_BV(ICIE1);
    // Then enable global interupts before this interrupt is finished, so the
    // music can interrupt the buffering sei();
...
ISR(TIMER2_OVF_vect) {
    // TIMER2 runs at 16mhz/255 (62,745Hz), so the timing for sample rate must
    // be manually counted
    switch (SR) {
        case 0:
            break;
...
ISR(TIMER1_OVF_vect) {
    ...
    if (!bitRead(optionByte, 4)) {
        ...
        if (volMod < 0) {
            *OCRnA[tt] = *OCRnB[tt] =
                buffer[whichBuff][buffCount] >> (volMod * -1);
        } else {
        ...
        if (volMod < 0) {
            *OCRnA[tt] = *OCRnB[tt] =
                buffer[whichBuff][buffCount] >> (volMod * -1);
            *OCRnA[tt2] = *OCRnB[tt2] =
                buffer[whichBuff][buffCount + 1] >> (volMod * -1);

        } else {
            *OCRnA[tt] = *OCRnB[tt] = buffer[whichBuff][buffCount] << volMod;
            *OCRnA[tt2] = *OCRnB[tt2] = buffer[whichBuff][buffCount + 1]
                                        << volMod;
        }
...
boolean TMRpcm::isPlaying() { return playing; }
...
void TMRpcm::play(char* filename, boolean which) { play(filename, 0, which); }

void TMRpcm::play(char* filename) { play(filename, 0, 0); }
...
void TMRpcm::play(char* filename, unsigned long seekPoint, boolean which) {
    ...
    if ((!playing && !playing2) || (playing && !playing2 && !which) ||
        (playing2 && !playing && which)) {
        stopPlayback();
        if (!wavInfo(filename)) {
            return;
        }  // verify its a valid wav file
...
void TMRpcm::setVolume(char vol, boolean which) { volMod = vol - 4; }
    ...
    if (playing2) {
        if (!bitRead(optionByte, 4)) {
...
#else
                *OCRnA[tt2] = *OCRnB[tt2] =
                    buffer2[whichBuff2][buffCount2] >> (volMod * -1);
#endif
            } else {
#if !defined(MODE2)
                *OCRnB[tt] = buffer2[whichBuff2][buffCount2] << volMod;
#else
                *OCRnA[tt2] = *OCRnB[tt2] = buffer2[whichBuff2][buffCount2]
                                            << volMod;
#endif
            }
            buffCount2++;
            ...
            if (volMod < 0) {
                ...
                *OCRnB[tt] = buffer2[whichBuff2][buffCount2] >> (volMod * -1);
                *OCRnA[tt] =
                    buffer2[whichBuff2][buffCount2 + 1] >> (volMod * -1);
#else
...
boolean TMRpcm::isPlaying() { return (playing | playing2); }
...
byte TMRpcm::metaInfo(boolean infoType, char* filename, char* tagData,
                      byte whichInfo) {
    if (ifOpen()) {
        noInterrupts();
    }
    ...
    if (infoType == 0) {  // if requesting LIST info, check for data at
                          // beginning of file first
        xFile.read((char*)tmpChars, 4);
...
//** This section reads the tags and gets the size of the tag data and its
// position
//** Should work with very long tags if a big enough buffer is provided
    ...
    while (xFile.position() < listEnd) {
        ...
        tagPos = xFile.curPosition();
            ...
            found = 0;
            //** This section checks to see if the tag we found is the one
            // requested
...
//**This section jumps to the next tag position if the requested tag wasn't
// found
#if !defined(SDFAT)
            if (infoType == 0) {
                if (!found) {
    ...
    *******************************/
    ...
    fSize = sFile.fileSize() - 8;
        ...
        seek(4);
        byte data[4] = {lowByte(fSize), highByte(fSize), fSize >> 16,
                        fSize >> 24};
        sFile.write(data, 4);
        byte tmp;
        seek(40);
    ...
    void TMRpcm::startRecording(char* fileName, unsigned int SAMPLE_RATE,
                                byte pin) {
        startRecording(fileName, SAMPLE_RATE, pin, 0);
    }

    void TMRpcm::startRecording(char* fileName, unsigned int SAMPLE_RATE,
                                byte pin, byte passThrough) {
        recording = passThrough + 1;
        setPin();
        if (recording < 3) {
            //*** Creates a blank WAV template file. Data can be written
            // starting at the 45th byte ***
...
/*** This section taken from wiring_analog.c to translate between pins and
 * channel numbers ***/
    ...
    defined(__AVR_ATmega644P__) || defined(__AVR_ATmega644PA__)
        ...
        if (recording > 1) {
            *TCCRnA[tt] = _BV(
                COM1A1);  // Enable the timer port/pin as output for passthrough
        }
        *ICRn[tt] =
            10 * (RESOLUTION_BASE /
                  SAMPLE_RATE);  // Timer will count up to this value from 0;
        *TCCRnA[tt] |=
            _BV(WGM11);  // WGM11,12,13 all set to 1 = fast PWM/w ICR TOP
        *TCCRnB[tt] =
            _BV(WGM13) | _BV(WGM12) | _BV(CS10);  // CS10 = no prescaling

        if (recording < 3) {  // Normal Recording
            *TIMSK[tt] |=
                _BV(OCIE1B) |
                _BV(OCIE1A);  // Enable the TIMER1 COMPA and COMPB interrupts
        } else {
            *TIMSK[tt] |=
                _BV(OCIE1B);  // Direct pass through to speaker, COMPB only
        }

        ADMUX |=
            _BV(REFS0) | _BV(ADLAR);  // Analog 5v reference, left-shift result
                                      // so only high byte needs to be read
        ADCSRB |=
            _BV(ADTS0) |
            _BV(ADTS2);  // Attach ADC start to TIMER1 Compare Match B flag
        byte prescaleByte = 0;

        if (SAMPLE_RATE < 18000) {
            prescaleByte = B00000110;
        }  // ADC division factor 64 (16MHz / 64 / 13clock cycles = 19230 Hz Max
           // Sample Rate )
        ...
        ADCSRA = prescaleByte;  // Adjust sampling rate of ADC depending on
                                // sample rate
